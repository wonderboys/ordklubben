import { AdminBreadcrumb, StatusBadge } from '@/components/admin/admin-ui';
...
export function WordDetailHeader({ word }: { word: WordDetailData }) {
  ...
  const summaryParts = [
    ...
    word.languageData?.partOfSpeech ? formatPartOfSpeech(word.languageData.partOfSpeech) : null,
  ...
  return (
    <header className="border-b border-print-ink/10 pb-4">
      <AdminBreadcrumb items={[{ label: 'Ord', href: '/admin/words' }, { label: word.answer }]} />
      <div className="mt-3 space-y-2">
        ...
        <p className="text-sm text-print-muted">{summaryParts.filter(Boolean).join(' · ')}</p>
      </div>
    </header>
  );
}
