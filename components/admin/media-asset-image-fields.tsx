    ...
    <>
      {filePath ? (
        <Field label="Bild" htmlFor={`media-image-preview-${mediaTypeFieldId}`}>
          ...
          >
            <Image src={filePath} alt="" fill className="object-cover" sizes="96px" />
          </div>
        </Field>
      ) : null}
      <Field
        label="Välj bild"
        htmlFor={`media-image-upload-${mediaTypeFieldId}`}
        hint="JPG, PNG eller WebP. Max 5 MB."
      >
        <FileInput
...
export function MediaAssetImagePreview({ filePath, altText }: MediaAssetImagePreviewProps) {
  return (
    <div className="mt-3">
      <p className="mb-1.5 text-xs font-medium uppercase tracking-[0.04em] text-print-muted">
        Bild
      </p>
