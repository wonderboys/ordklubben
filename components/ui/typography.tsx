...
export function PageTitle({ className, variant = 'page', ...props }: PageTitleProps) {
...
export function BodyText({ className, variant = 'intro', ...props }: BodyTextProps) {
  return (
    <p
      className={cn(
...
export function MonoLabel({ className, muted = false, ...props }: MonoLabelProps) {
  return (
    <p
      className={cn('print-mono', muted ? 'text-print-muted' : 'text-print-ink', className)}
      {...props}
    />
  );
}
